...
- Toggle QEMU ISA debug console via `QEMU_ISA_DEBUGCON=true|false` with `make qemu`.
- RAMFS contents are embedded; `programs/*.asm` assemble to `ramfs/bin/*` during build.
